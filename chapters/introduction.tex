...
Produkt dostosowywany jest pod uprzednio zadane zasoby, a w sytuacji gdy dany efekt skutkuje w spadkach wydajności - jest usuwany.
...
Pierwszy, coraz częściej wykorzystywany polega na użyciu procesora grafiki w celu przeprowadzenia dowolnych obliczeń o charakterze współbieżnym.
Służy to odciążeniu zasobów procesora CPU przy jednoczesnym zwiększeniu wydajności ze względu na oddelegowanie pracy do jednostki będącej lepiej przystosowaną do typu zadania.
Idea ta nazywana jest GPGPU \eng{General-Purpose GPU}, a więc skupia się na oddaniu w ręce karty graficznej pracy nienastawionej bezpośrednio na tworzenie grafiki.
Drugi, tradycyjny typ zadań to skorzystanie z wbudowanego potoku renderowania.
Choć skorzystaniu z niego z poziomu różnych API towarzyszą subtelne różnice, to kroki obecne podczas procesu pozostają niezmienne.
\par
...
Część z nich nie podlega żadnej konfiguracji, inne mogą być parametryzowane w ograniczony sposób, kluczowe są w pełni modyfikowalne, a logika ich działania dostarczana jest w formie programu komputerowego. 
\begin{figure}[H]
    \begin{center}
