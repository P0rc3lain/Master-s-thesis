...
W przypadku osi \textit{y} kolejne, następujące po sobie liczby idą w górę.
Wprowadzenie trzeciej koordynaty i rozszerzenie układu na potrzeby trójwymiarowości pozostawia pewną dowolność.
...
\end{figure}
\par
Punkt składający się na fragment siatki modelu nanoszony na osie opisany jest w odniesieniu do pewnego układu współrzędnych.
Ten jednak, jak wyjaśniono w poprzedniej sekcji zagnieżdżony może być w innym.
Z tego względu w celu określenia pozycji w stosunku do globalnego układu konieczne jest przeprowadzenia szeregu konwersji.
Spośród wielu występujących w bibliotekach graficznych wyróżnić możemy kilka najważniejszych:
\begin{figure}[H]
    \begin{center}
...
\paragraph{Układ styczny} którego wektory bazowe składają się z wektora normalnego, tangensa oraz bitangensa zadanego wierzchołka siatki.
...
\paragraph{Układ modelu} powiązany z siatką modelu jako całością bądź jej mniejszymi fragmentami.
\paragraph{Układ świata} stanowi korzeń hierarchii w odniesieniu do którego opisane są pozostałe układy.
\paragraph{Układ kamery} osadza kamerę w początku układu współrzędnych. Z jego poziomu generowana jest klatka obrazu.
\paragraph{Układ przycinania} jest bezpośrednim następcą układu kamery, z którego przejście nastąpiło na skutek projekcji.
...
\begin{figure}[H]
    ...
    \label{fig:ndc-metal}
\end{figure}
\paragraph{Układ okna} to dyskretna przestrzeń dwuwymiarowa stojąca na szczycie w hierarchii generowania grafiki, rozciągająca się na całej powierzchni warstwy użytej do wyświetlenia obrazu.
\begin{figure}[H]
    \begin{center}
