...
\end{figure}
% Omów różne układy współrzędnych
...
Współczesne rozwiązania, takie jak Metal, Direct X oraz Vulkan pochwalić mogą się szerszym wachlarzem funkcjonalności.
...
\begin{enumerate}
    ...
    \item Wczytywanie asetów
    \item Aktualizacja asetów
    \item Prezentacja
...
Jeśli jednostka graficzna obsługuje technologię API, wówczas zakładając posiadanie podstawowych narzędzi developerskich można jej użyć we własnej produkcji.
...
Idea określana jest mianem GPGPU \eng{General-Purpose GPU}, a więc służy wykonaniu pracy nienastawionej bezpośrednio na tworzenie grafiki.
...
\par
Potok renderowania przedstawiony i omówiony zostanie na podstawie ilustracji dotyczącej specyfikacji technicznej technologii Metal.
...
Dane pobierane są z określonego przez użytkownika bufora danych i interpretowane jako punkty, w przestrzeni dwu, trzy lub czterowymiarowej.
Dalszemu procesowaniu podlegać mogą w niezmienionej formie, interpretowane jako linie, trójkąty lub łączone w kilka innych figur.
Określenie w jaki sposób mają być ze sobą łączone ma kluczowy wpływ na ich dalszy wygląd po procesie rasteryzacji.
...
Jeśli pozycja wierzchołka znajduje się wewnątrz graniastosłupa definiującego ten układ wierzchołek kontynuuje drogę przez potok, w przeciwnym razie jest odrzucany.
...
Zamiast używać pojedynczego punktu do określenia czy figura pokrywa dany fragment używana jest ich większa ilość.
...
\begin{figure}[H]
    \begin{center}
        \includegraphics[width=3.5cm]{images/pipeline/multisampling.png}
        \caption{Zobrazowanie idei multisamplingu}
    \end{center}
...
\begin{figure}[H]
    \begin{center}
        \includegraphics[width=6cm]{images/pipeline/binary-mask.png}
...
Jego celem jest sprawdzenie czy dany fragment nie jest przesłonięty przez inne.
Z tego względu karta graficzna musi skorzystać z dodatkowe bufora.
...
W najczęstszym przypadku zadanie testu polega na wykonaniu pojedynczego porównania - wartości obecnej w buforze z aktualną obliczoną dla obecnie procesowanego fragmentu.
\paragraph*{Wynik widoczności \eng{Visibility Result}}
Wynik z działania testu głębi zachowany jest w postaci liczby binarnej.
...
Wykonywane jest ono zawsze, jednak widoczne rezultaty przynosi w przypadku gdy kolor zadanego fragmentu charakteryzuje się stopniem przeźroczystości.
