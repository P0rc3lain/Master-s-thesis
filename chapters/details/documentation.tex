...
Stworzona biblioteka posiada dokumentację występującą w dwóch formach.
...
Traktowany może być jako wizytówka.
...
Zawierają one opisy parametrów wejściowych, wartości zwracanych, a także samego celu stojącego za użyciem danej metody czy klasy.
Objęły one publiczne komponenty, a dzięki dołączeniu dodatkowych plików poruszyły także zagadnienie architektury.
\lstinputlisting[language=Swift, caption=Dokumentacja kodu w języku Swift]
                {code/docc.swift}
\par
...
Zdecydowano się wprowadzić podział na podstawie ich semantyki.
Rozróżnienie występuje w zależności od funkcjonalności do jakiej dany symbol należy.
Struktura dokumentacji przypomina nieco tę repozytorium i wśród wielu innych wymienić można takich zagadnienia jak: zarządzanie sceną, elementy sceny, interfejs graficzny.
...
\figh{images/docs/camera_docs.png}
     {Dokumentacja na przykładzie klasy kamery}
     {fig:camera-docs}
     {10cm}
