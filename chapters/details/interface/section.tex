...
\code{Swift}
     {Idea konfiguracji}
     {code/configuration/PNDefaults.swift}
\par
...
Przykładowo konfiguracja modelu oświetlenia dla źródeł punktowo-kierunkowych zamknięta jest w strukturę \textit{PNOmniLighting}, a kierunkowych w \textit{PNDirectionalLighting}.
Zagnieżdżenie wewnątrz \textit{PNDefaults} sprawia, że pomimo iż wszystkie struktury są publiczne to dostęp do nich jest ograniczony.
...
\code{Swift}
     {Konfiguracja silnika w kodzie klienckim}
     {code/configuration/PNDefaults_example.swift}
