...
Dzięki temu wyspecjalizowane są one w kierunku pełnionej przez siebie funkcjonalności i łatwiej można je analizować niż w przypadku alternatywnego podejścia, gdzie węzeł jednocześnie mógłby przechowywać wszystkie typy.
\subsubsection{Transformacja}
Podstawowym typem węzłów są węzły transformacji.
...
Animacje omówione zostaną w dalszej części pracy.
...
\figh{images/pnengine/scene/camera.png}
     ...
     {8cm}
\par
Sam interfejs węzła w stosunku rozszerza podstawowy interfejs jedynie o dodanie pojedynczej zmiennej \longpause referencję do kamery.
\lstinputlisting[language=Swift, caption=Węzeł kamery]{code/PNCameraNode.swift}
\subsubsection{Model}
...
\lstinputlisting[language=Swift, caption=Węzeł kamery]{code/PNMeshNode.swift}
...
\lstinputlisting[language=Swift, caption=Węzeł kamery]{code/PNRiggedMeshNode.swift}
...
\lstinputlisting[language=Swift, caption=Węzeł kamery]{code/PNAmbientLightNode.swift}
...
\lstinputlisting[language=Swift, caption=Węzeł kamery]{code/PNSpotLightNode.swift}
We wszystkich przypadkach interfejsy są zwięzłe i oczekują jedynie referencji do wybrangeo typu światła.
\lstinputlisting[language=Swift, caption=Węzeł kamery]{code/PNOmniLightNode.swift}
...
\figh{images/pnengine/scene/particle.png}
     ...
     {fig:scene_particle}
     {4cm}
\lstinputlisting[language=Swift, caption=Węzeł kamery]{code/PNParticleNode.swift}