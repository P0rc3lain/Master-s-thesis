...
Użytkownik jest w stanie na własną rękę implementować interfejs \textit{PNSceneNode} do personalizacji zachowania, jednak nie jest to konieczne do skorzystania z biblioteki.
W celu wygenerowania obrazu i aranżacji sceny posłużyć się może szeregiem predefiniowanych węzłów, które rozróżnić można na kilka typów.
...
Podobnie jak w innych przypadkach deformacje siatki wynikające z ruchu szkieletu połączyć można z techniką animacji na podstawie klatek kluczowych.
\code{Swift}
     {Interfejs węzła siatki obsługującej animację szkieletową}
     {code/nodes/PNRiggedMeshNode.swift}
\subsubsection{Światła}
Aby obraz w ogóle mógł powstać konieczne są źródła światła sprawiące, że kamera zarejestruje kształty modeli.
\code{Swift}
