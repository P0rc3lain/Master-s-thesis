...
Choć interfejsy API posiadają subtelne różnice, to kroki obecne w procesie pozostają niezmienne.
\par
...
Rzutowanie z układu współrzędnych kamery na oczekiwaną przestrzeń pozostawione jest programiście.
...
Piksele służące do opisu figury po procesie rasteryzacji określane są mianem fragmentów.
...
Dzięki temu obserwator jest w stanie dostrzec wpływ oświetlenia na usytuowany wewnątrz sceny model.
...
W takim wypadku istnieje możliwość określenia prostokątnego obszaru, który powinien zostać utrwalony.
...
Realizowanym celem jest wygładzenie krawędzi pomiędzy różnymi obiektami na obrazie wyjściowym.
...
Cieniowanie fragmentów wciąż wykonywane jest tylko i wyłącznie dla pojedynczego elementu, jednak uzyskany kolor wyjściowy mieszany jest z tłem w oparciu o dane z próbkowania dla większej liczby punktów.
\figh{images/pipeline/multisampling.png}
     {Zobrazowanie idei wielopróbkowania}
     {fig:multisampling}
     {6cm}
\paragraph*{Test szablonu \eng{Stencil Test}}
Poza wspomnianym przycinaniem użytkownik ma dodatkową możliwość ograniczenia zaprezentowanej powierzchnii obrazu.
...
W podstawowym przypadku na bufor wyjścia składają się kolor, głębia oraz szablon. 
