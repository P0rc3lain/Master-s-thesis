...
Android, rozwijany przez Google opiera się o nieco odmienne podejście na dostarczanie dodatkowych funkcjonalności niż to przyjęte w Apple.
Producent aktywnie udoskonala system, dostarcza komponenty do budowy aplikacji, ale długi czas nie zapewniał bibliotek ułatwiających pracę w wyspecjalizowanych dziedzinach.
...
Bibliotekę do obsługi AR, a wraz z nią silnik graficzny Filament, stworzony na potrzeby wyświetlania grafiki.
\par
...
Filament stworzony został z naciskiem na opisany scenariusz.
\par
Oferowany jest jako silnik do renderowania.
W celu ułatwienia procesu importu modeli posiada moduł do wczytywania.
Są to główne, ale i jedyne funkcjonalności.
\par
Choć wspierany jest szereg systemów największy nacisk położony jest na Android.
...
Dziwi, że Google wprawdzie umiejscowiło repozytorium pod skrzydłami swojej organizacji, ale zaznacza, że wsparcie nie jest oficjalne.
\par
...
\par
Dokumentacja oparta jest o trzy główne filary.
...
Przytłaczający może wydawać się stopień włożonego wysiłku, poświęconego by przedstawić zjawiska fizyczne przy pomocy aparatu matematycznego.
Starano oddać się kompletny proces, bez pomijania kroków pośrednich.
Drugi z filarów to funkcjonalne przykłady użycia.
Jest ich kilka, w formie aplikacji dla każdej z platform.
...
Wprawdzie Filament zdolny jest do pracy zarówno na OpenGL, jak i Metal, natomiast włożony nakład pracy wydaje się ograniczać tylko do rozszerzenia obsługi o wspomniane API.
...
Filament jest silnikiem graficznym wykonanym przez grafików dla grafików.
Przeciętny programista ekosystemu Apple, pragnący tylko skorzystać z funkcjonalności może mieć trudności w efektywnym wykorzystaniu.
\figh{images/engines/filament/helmet.jpeg}
     {Przykład klatki wygenerowanej przez silnik Filament}
     {fig:filament-helmet}
     {7cm}
% TODO: Można opisać, że dystrybucja jest w klasycznym stylu C, 
% otrzymuje się osobno plik binarny oraz pliki nagłówkowe, nie ma pojedynczego pliku