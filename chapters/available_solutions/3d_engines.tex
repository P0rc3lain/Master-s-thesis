...
Możliwe minimalizowanie zjawiska jest newralgiczne, każda strata płynności wynikająca z wzmożonego nakładu obliczeń procesora implikować będzie konieczność obniżenia jakości grafiki celem utrzymania pożądanej prędkości animacji. 
\par
...
Android, którego rozwój prowadzi Google opiera się o nieco odmienne podejście na dostarczanie dodatkowych funkcjonalności.
Producent aktywnie rozwija system, dostarcza komponenty w oparciu o które można budować aplikacje, ale długi czas pozostawiał społeczności projektowanie i rozwój bibliotek ułatwiających pracę w wyspecjalizowanych dziedzinach.
...
Filament stworzony został dokładnie z tym myślą o tym scenariuszu.
\par
...
\par
Projekt wspiera szereg systemów, ale największy nacisk położony jest na Android.
Pomimo iż stworzony został w C++ integracja z Kotlinem stoi na wysokim poziomie.
...
Zupełnie wyjątkowy jest jednak sposób w jaki zaimplementowane je w bibliotece.
Zadbano, aby każdy konfigurowalny parametr miał fizyczne znaczenie.
Nie ma możliwości nadania kierunkowemu światłu imitującemu słońce wartości na podstawie czynników koloru czerwonego, zielonego oraz niebieskiego.
...
Starano oddać się kompletny proces, bez pomijania żadnych aspektów.
...
Ostatni aspekt dokumentacji - kodu oraz interfejsu niestety zawodzi.
Nie udostępniono użytkownikom jej za pośrednictwem internetu, uzyskać można ją tylko przy budowie aplikacji.
Niestety w głównej mierze ogranicza się ona do jednozdaniowych opisów.
\par
...
Nie posiada ona dedytkowanego interfejsu dla platformy apple i wszystkie jej funkcjonalności wymagają użycia C++.
Wobec tego, aby skorzystać z biblioteki koniecznym jest oprzeć część aplikacji o Objective-C i zastosować interfejsowanie.
\par
...
Przeciętny programista ekosystemu Apple, który chciałby tylko skorzystać z funkcjonalności może mieć trudności efektywnym wykorzystaniu.
\begin{figure}[H]
    \begin{center}
...
% Napisać, że scenekit jest wyposażony w edytor sceny
\subsection{Pozostałe silniki}
% Jeśli nie mam zamiaru opisywać unreal engine i godot to powinienem tutaj o nich wspomnieć
