...
W przypadku urządzeń mobilnych jest to OpenGL ES 3.0, które pomimo zbieżności pozostaje podzbiorem stacjonarnego odpowiednika.
W 2023 roku nadal istnieje możliwość stworzenia aplikacji opartej o OpenGL, choć rozwiązanie uznane jest jako przestarzałe i nierekomendowane od 2018 roku.
\par
...
W przeciwieństwie do nowoczesnych API synchronizujących zadania CPU oraz GPU w zminimalizowany sposób OpenGL dokonuje tego znacznie częściej.
...
\par
Kolejną niewątpliwą wadą jest sama architektura.
...
W teorii dwa lub więcej wątków może pracować współbieżnie za pomocą różnych kontekstów, ale operacje w sekcji krytycznej są nieefektywne, ponieważ każdorazowa zmiana stanu w GPU jest kosztowna czasowo.
\par
...
Pomimo odchodzenia branży od OpenGL społeczność stale wskazuje je jako najbardziej przystępne spośród wszystkich dostępnych.
\par
Platformy Apple zawsze były znacząco opóźnione pod względem wspieranej wersji OpenGL w stosunku do standardu.
Aktualna posiada kilka istotnych braków.
Przykładami jest niemożliwość skorzystania z programów obliczeniowych i tesalacyjnych.
% TODO: Napisać, że opengl jest goły i dużo trzeba robić na własną rękę - tak jak kompilacja warunkowa
