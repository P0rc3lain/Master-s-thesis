@objc func handleTap(_ sender: UITapGestureRecognizer? = nil) {
        guard state.checkState != .stalemate && 
              state.checkState != .checkmate else {
            return
        }
        let allNodes = engine.scene.rootNode.all()
        let camera = allNodes.compactMap {
        ...
        if state.expectation == .piece {
            ...
            let parser = PieceParser()
            let pieceS = parser.create(literal: piece?.data.name ?? "")
            ...
            let fieldS = parser.create(literal: field?.data.name ?? "")
        ...
        if selectedAfter != selected {
            if let selected = selected {
                manipulator.deselect(scene: engine.scene,
                                     piece: selected)
            }
            if let selectedAfter = selectedAfter {
                manipulator.select(scene: engine.scene, 
                                   piece: selectedAfter)
            }
        }
